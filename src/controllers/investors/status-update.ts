...
export const investorStatusUpdate = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const coordinator = await investorsService.getInvestorByIdWithoutStatus(
      req.params.id,
      client_id
    );
    ...
    await investorsService.updateStatus(req.params.id, req.body.status);

    const data = formatResponse(
      200,
      false,
      "Investor status updated successfully.",
      null
    );
