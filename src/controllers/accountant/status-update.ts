...
export const accountantStatusUpdate = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const coordinator = await accountantService.getAccountantByIdWithoutStatus(
      req.params.id,
      client_id
    );
    ...
    await accountantService.updateStatus(req.params.id, req.body.status);
