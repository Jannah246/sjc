...
const assignedCampsListClientPackageSchema = Joi.object({
  client_package_id: Joi.string().required().messages({
...
export const assignedCampsListClientPackageValidator = async (
  ...
  const { error } = assignedCampsListClientPackageSchema.validate(req.query, {
    abortEarly: false,
  });
  if (error) {
    const { details } = error;
