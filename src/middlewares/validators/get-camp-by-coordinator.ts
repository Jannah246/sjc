...
const getCampByCoordinatorSchema = Joi.object({
  coordinator_id: Joi.string().required().messages({
    "any.required": "Coordinator id field is required.",
    "string.empty": "Coordinator id field is required.",
  }),
...
export const getCampByCoordinatorValidator = async (
  ...
  const { error } = getCampByCoordinatorSchema.validate(req.query, {
    abortEarly: false,
  });
  if (error) {
    const { details } = error;
